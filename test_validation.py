...
class SUNOValidation:
    ...
    async def run(self):
        ...
        logger.info("\n" + "=" * 70)
        logger.info("  SUNO END-TO-END VALIDATION TEST")
        logger.info("  Reality Test: Prove automation works before launch")
        logger.info("=" * 70 + "\n")
    ...
    async def _step_1_whop_connection(self):
        ...
        try:
            if not config.WHOP_API_KEY:
                ...
                logger.info("       Action: Add WHOP_API_KEY to .env file")
            ...
            try:
                ...
                if not company_id:
                    logger.info(f"  [FAIL] WHOP_COMPANY_ID not set in .env")
                    logger.info("       Action: Add your Whop company ID to .env file")
                    self.results['step1_whop'] = False
                    return
                ...
                if resp.status_code == 200:
                    ...
                    logger.info(f"  [PASS] Whop API reachable and authenticated")
                    logger.info(f"       Campaigns endpoint: OK (found {len(campaigns)} campaigns)")
                    self.results['step1_whop'] = True

                elif resp.status_code == 401:
                    logger.info(f"  [FAIL] HTTP 401: API key is not authorized")
                    logger.info("       Action: Verify WHOP_API_KEY in .env is correct")
                    self.results['step1_whop'] = False

                elif resp.status_code == 403:
                    logger.info(f"  [FAIL] HTTP 403: API key lacks required permissions")
                    logger.info("       Action: Check that API key has 'campaigns' scope in Whop dashboard")
                    self.results['step1_whop'] = False

                elif resp.status_code == 404:
                    logger.info(f"  [FAIL] HTTP 404: Endpoint not found")
                    logger.info(f"       Attempted: GET /campaigns?limit=100")
                    logger.info("       This suggests: incorrect API path, outdated Whop API version, or")
                    logger.info("                       endpoint doesn't exist for this API key's account type")
                    logger.info("       Action: Verify correct Whop API endpoint in official docs")
                    try:
                        error_detail = resp.json()
                        logger.info(f"       Response: {error_detail}")
                    except:
                        pass
                    self.results['step1_whop'] = False

                else:
                    logger.info(f"  [FAIL] HTTP {resp.status_code}: Unexpected response from Whop")
                    try:
                ...
                logger.info(f"  [FAIL] Could not reach Whop API: {e}")
                self.results['step1_whop'] = False

        except WhopAuthError as e:
            logger.info(f"  [FAIL] Whop auth error: {e}")
            logger.info("       Action: Verify WHOP_API_KEY is correct in .env")
            self.results['step1_whop'] = False
        except Exception as e:
            logger.info(f"  [FAIL] Whop validation error: {e}")
            logger.info("       Check: Network connection? API endpoint available?")
            self.results['step1_whop'] = False
        ...
        try:
            ...
            if clip_files:
                ...
                try:
                    ...
                    logger.info(f"  DEBUG: add_clip returned: {clip_db_id}")
                    ...
                    logger.info(f"  [PASS] Test clip prepared and saved to DB: {test_file.name}")
                    logger.info(f"       Clip ID in DB: {self.test_clip_id}")
                    self.results['step2_clips'] = True
                except Exception as e:
                    import traceback
                    logger.info(f"  [FAIL] Error saving clip to DB: {e}")
                    traceback.print_exc()
                    self.results['step2_clips'] = False
            else:
                logger.info(f"  [FAIL] No .mp4 files in {config.CLIPS_INBOX}")
                logger.info(f"       Action: Add test video to clips/inbox/")
                self.results['step2_clips'] = False

        except Exception as e:
            logger.info(f"  [FAIL] Error checking clips: {e}")
            self.results['step2_clips'] = False
    ...
    async def _step_3_post_clip(self):
        ...
        logger.info("[STEP 3/5] Posting clip to platforms...\n")

        if not self.test_clip_id:
            logger.info("  [SKIP] No clip to post (see step 2)")
        ...
        try:
            ...
            if not has_creds:
                ...
                logger.info(f"  [FAIL] Missing credentials: {', '.join(missing)}")
                logger.info(f"       Action: Set in .env file")
                self.results['step3_post'] = False
                return

            logger.info(f"  Credentials verified: {config.PLATFORMS}")
            ...
            async with PlatformPoster() as poster:
                ...
                if "youtube" in successful_platforms:
                    logger.info(f"  [PASS] Posted to YouTube (primary platform for MVP)")
                    if success_count == total_platforms:
                        logger.info(f"       Bonus: Also succeeded on {success_count}/{total_platforms} platforms")
                    else:
                        other_success = successful_platforms - {"youtube"}
                        if other_success:
                            logger.info(f"       Also posted: {', '.join(other_success)}")
                        other_failed = set(config.PLATFORMS) - successful_platforms
                        if other_failed:
                            logger.info(f"       Phase 2 (anti-bot detection): {', '.join(other_failed)}")
                    self.posted_urls = {
                        p: r.url for p, r in results.items() if r.success
                    }
                    self.results['step3_post'] = True
                else:
                    logger.info(f"  [FAIL] YouTube posting failed (required for MVP)")
                    for platform, result in results.items():
                        logger.info(f"       {platform}: {result.error}")
                    self.results['step3_post'] = False

        except Exception as e:
            logger.info(f"  [FAIL] Posting error: {e}")
            self.results['step3_post'] = False
    ...
    async def _step_4_submit_whop(self):
        ...
        logger.info("[STEP 4/5] Submitting clip URL back to Whop...\n")

        if not self.posted_urls:
            logger.info("  [SKIP] No posted URLs to submit (see step 3)")
        ...
        try:
            ...
            if not campaigns:
                logger.info("  [BLOCKED] Whop API connected but no campaigns available")
                logger.info("       Status: Waiting for campaign creation in Whop dashboard")
                logger.info("       Action: Create a campaign in Whop dashboard to enable submissions")
                logger.info("       Once created, clip submissions will work automatically")
                logger.info("       Note: This is a blocking condition, not a failure")
                self.results['step4_submit'] = False  # Blocked, not ready yet
                return

            test_campaign = campaigns[0]
            logger.info(f"  Submitting to campaign: {test_campaign['name']}")
            ...
            if result['success']:
                logger.info(f"  [PASS] Submission successful")
                logger.info(f"       Campaign: {test_campaign['name']}")
                logger.info(f"       URL: {first_url}")
                self.submission_result = result
                self.results['step4_submit'] = True
            else:
                logger.info(f"  [FAIL] Submission failed: {result['error']}")
                self.results['step4_submit'] = False

        except Exception as e:
            logger.info(f"  [FAIL] Submit error: {e}")
            self.results['step4_submit'] = False
    ...
    async def _step_5_track_earnings(self):
        ...
        logger.info("[STEP 5/5] Verifying posted clips are tracked...\n")

        try:
            if not self.posted_urls:
                logger.info(f"  [SKIP] No posted clips to track (see step 3)")
            ...
            with sqlite3.connect(queue.db_path) as conn:
                ...
                if self.test_clip_id:
                    ...
                    if clip:
                        logger.info(f"  DEBUG: Clip in DB - ID: {clip['id']}, Status: {clip['status']}, File: {clip['filename']}")
                    else:
                        logger.info(f"  DEBUG: Clip ID {self.test_clip_id} NOT FOUND in DB")
                ...
                logger.info(f"  DEBUG: Clips downloaded today: {len(all_today)}")
                for c in all_today:
                    logger.info(f"    - ID: {c['id']}, File: {c['filename']}, Status: {c['status']}")
            ...
            if clips_posted > 0:
                logger.info(f"  [PASS] Posted clips are being tracked")
                logger.info(f"       Clips posted today: {clips_posted}")
                logger.info(f"       Total views: {today.get('total_views', 0):,}")
                logger.info(f"       Estimated earnings: ${today.get('total_earnings', 0):.2f}")
                self.results['step5_tracking'] = True
            else:
                logger.info(f"  [PARTIAL] Tracker exists but posted clip not yet recorded")
                logger.info(f"       Action: Check if tracker needs to be updated after posting")
                logger.info(f"       Expected: clips_posted >= {len(self.posted_urls)}")
                logger.info(f"       Actual: clips_posted = {clips_posted}")
                # This is partial - tracker works but didn't capture our posted clip
                self.results['step5_tracking'] = False

        except Exception as e:
            import traceback
            logger.info(f"  [FAIL] Tracking verification error: {e}")
            traceback.print_exc()
            self.results['step5_tracking'] = False
    ...
    def _print_summary(self):
        ...
        for step_key, step_label, req_type in steps:
            ...
            if result:
                ...
                status = "FAIL"
                symbol = "N"
            logger.info(f"  [{symbol}] {step_label:<30} [{status}]")

        logger.info(f"\nMVP Readiness: {required_passes}/{required_total} required steps passed")
        if blocked > 0:
            logger.info(f"              {blocked} blocking condition (campaign creation needed)")
        logger.info()

        # MVP requires all 5 core steps to work (Step 4 blocked by missing campaign is acceptable)
        if required_passes == required_total:
            logger.info("*** SUNO IS MVP-READY - CORE AUTOMATION WORKS ***")
            logger.info("    YouTube posting + earnings tracking functional")
            if blocked > 0:
                logger.info("    Whop submission ready once campaigns are created")
            logger.info("\n    Ready for MVP launch!\n")
            return True
        else:
            logger.info("!!! SUNO IS NOT YET READY !!!")
            failing = [label for key, label, req in steps if req == 'required' and not self.results.get(key, False)]
            logger.info(f"   Critical failures: {', '.join([l.split('. ')[1] for l in failing])}\n")
            return False
